...
class CRNetLCData(VideoData):
    def __init__(self, data_root, img_dir, albedo_img_dir, shading_img_dir, audio_dir, label_file, transform=None, sample_size=100):
        super().__init__(data_root, img_dir, label_file, audio_dir)
        self.transform = transform
        self.sample_size = sample_size
        self.albedo_img_dir_ls = self.get_face_img_dir(albedo_img_dir)
        self.shading_img_dir_ls = self.get_face_img_dir(shading_img_dir)
    ...
    def __getitem__(self, idx):
        glo_img, albedo_img, shading_img, idx = self.get_imgs(idx)  # in case the idx changed
        wav_aud = self.get_wav_aud(idx)
        anno_score = self.get_ocean_label(idx)
        anno_cls_encode = self.cls_encode(anno_score)
        ...
        if self.transform:
            glo_img = self.transform["frame"](glo_img)
            albedo_img = self.transform["face"](albedo_img)
            shading_img = self.transform["face"](shading_img)
        wav_aud = torch.as_tensor(wav_aud, dtype=glo_img.dtype)
        anno_score = torch.as_tensor(anno_score, dtype=glo_img.dtype)
        anno_cls_encode = torch.as_tensor(anno_cls_encode)

        sample = {
            "glo_img": glo_img, "albedo_img": albedo_img, "shading_img": shading_img, "wav_aud": wav_aud,
            "reg_label": anno_score, "cls_label": anno_cls_encode
        }
        return sample
    ...
    def get_imgs(self, idx):
        ...
        img_index = random.choice(separate)
        try:
            albedo_img_pt = albedo_imgs[img_index]
            shading_img_pt = shading_imgs[img_index]
        except IndexError:
            albedo_img_pt = albedo_imgs[0]
            shading_img_pt = shading_imgs[0]
        glo_img_pt = self._match_img(albedo_img_pt)

        albedo_img_arr = Image.open(albedo_img_pt).convert("RGB")
        shading_img_arr = Image.open(shading_img_pt).convert("RGB")
        glo_img_arr = Image.open(glo_img_pt).convert("RGB")

        return glo_img_arr, albedo_img_arr, shading_img_arr, idx
...
class AllFrameCRNetLCData(CRNetLCData):

    def __getitem__(self, idx):
        glo_img_ls, albedo_img_ls, shading_img_ls, idx = self.get_imgs(idx)
        wav_aud = self.get_wav_aud(idx)
        anno_score = self.get_ocean_label(idx)

        if self.transform:
            glo_img_ls = [self.transform["frame"](img) for img in glo_img_ls]
            albedo_img_ls = [self.transform["face"](img) for img in albedo_img_ls]
            shading_img_ls = [self.transform["face"](img) for img in shading_img_ls]
        wav_aud = torch.as_tensor(wav_aud, dtype=glo_img_ls[0].dtype)
        anno_score = torch.as_tensor(anno_score, dtype=glo_img_ls[0].dtype)

        sample = {
            "glo_img": glo_img_ls,
            "albedo_img": albedo_img_ls,
            "shading_img": shading_img_ls,
            "wav_aud": wav_aud,
            "reg_label": anno_score,
        }
        return sample
    ...
    def get_imgs(self, idx):
        ...
        # separate = np.linspace(0, len(loc_imgs), self.sample_size, endpoint=False, dtype=np.int16)
        separate = list(range(len(albedo_img_ls)))
        for img_index in separate:
            try:
                albedo_img_pt = albedo_imgs[img_index]
                shading_img_pt = shading_imgs[img_index]
            ...
            albedo_img_ls.append(albedo_img_pt)
            shading_img_ls.append(shading_img_ls)
            glo_img_ls.append(glo_img_pt)

        albedo_img_obj_ls = [Image.open(albedo_img) for albedo_img in albedo_img_ls]
        shading_img_obj_ls = [Image.open(shading_img) for shading_img in shading_img_ls]
        glo_img_obj_ls = [Image.open(glo_img) for glo_img in glo_img_ls]
        return glo_img_obj_ls, albedo_img_obj_ls, shading_img_obj_ls, idx
...
@DATA_LOADER_REGISTRY.register()
def crnet_lc_data_loader(cfg, mode=None):
    assert (mode in ["train", "valid", "test", "full_test"]), \
        " 'mode' only supports 'train', 'valid', 'test' and 'full_test' "
    ...
    if mode == "train":
        dataset = CRNetLCData(
            data_cfg.ROOT,  # "../datasets",
            data_cfg.TRAIN_IMG_DATA,  # "image_data/train_data",
            data_cfg.TRAIN_IMG_ALBEDO_DATA,  # "image_data/train_data_face",
            data_cfg.TRAIN_IMG_SHADING_DATA,  # "image_data/train_data_face",
            data_cfg.TRAIN_AUD_DATA,  # "voice_data/train_data",  # default train_data_244832 form librosa
            data_cfg.TRAIN_LABEL_DATA,  # "annotation/annotation_training.pkl",
            transforms,
        )
        ...
        dataset = CRNetLCData(
            data_cfg.ROOT,  # "../datasets",
            data_cfg.VALID_IMG_DATA,  # "image_data/train_data",
            data_cfg.VALID_IMG_ALBEDO_DATA,  # "image_data/train_data_face",
            data_cfg.VALID_IMG_SHADING_DATA,  # "image_data/train_data_face",
            data_cfg.VALID_AUD_DATA,  # "voice_data/train_data",  # default train_data_244832 form librosa
            data_cfg.VALID_LABEL_DATA,  # "annotation/annotation_training.pkl",
            transforms,
        )
    elif mode == "full_test":
        return AllFrameCRNetLCData(
            data_cfg.ROOT,  # "../datasets",
            data_cfg.TEST_IMG_DATA,  # "image_data/train_data",
            data_cfg.TEST_IMG_ALBEDO_DATA,  # "image_data/train_data_face",
            data_cfg.TEST_IMG_SHADING_DATA,  # "image_data/train_data_face",
            data_cfg.TEST_AUD_DATA,  # "voice_data/train_data",  # default train_data_244832 form librosa
            data_cfg.TEST_LABEL_DATA,  # "annotation/annotation_training.pkl",
            transforms,
        )
